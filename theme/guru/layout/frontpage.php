...
if (isloggedin()) {
    ...
    */
    ...
    $disablefrontpageloginbox = false;
    if (theme_guru_get_setting('disablefrontpageloginbox', true) == true) {
        $disablefrontpageloginbox = true;
        $extraclasses[] = 'disablefrontpageloginbox';
    }
    $bannerheading = '';
    if (!empty($PAGE->theme->settings->bannerheading)) {
        $bannerheading = theme_guru_get_setting('bannerheading', true);
    }
    $bannercontent = '';
    if (!empty($PAGE->theme->settings->bannercontent)) {
        $bannercontent = theme_guru_get_setting('bannercontent', true);
    }
    $bodyattributes = $OUTPUT->body_attributes($extraclasses);
    $regionmainsettingsmenu = $OUTPUT->region_main_settings_menu();
    $templatecontext = [
    ...
    echo $OUTPUT->render_from_template('theme_boost/frontpage_guest', $templatecontext);
